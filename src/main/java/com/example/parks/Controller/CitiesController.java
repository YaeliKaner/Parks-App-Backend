...
public class CitiesController {
    ...
    @GetMapping({"/getCityContainingLetters", "/getCityContainingLetters/{text}"})
    public ResponseEntity<List<Cities>> getCityContainingLetters(@PathVariable(required = false) String text) throws IOException {
        List<Cities> cities = (text == null || text.trim().isEmpty()) ?
                citiesRepository.findAllByOrderByName() :
                citiesRepository.findByNameContainingIgnoreCase(text);
        return new ResponseEntity<>(cities, HttpStatus.OK);
    }
