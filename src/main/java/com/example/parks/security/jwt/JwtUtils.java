...
public class JwtUtils {
    ...
    public ResponseCookie generateJwtCookie(CustomUserDetails userPrincipal) {
        String jwt = generateTokenFromUserEmail(userPrincipal.getUsername());
    ...
    public ResponseCookie getCleanJwtCookie(){
        ResponseCookie cookie = ResponseCookie.from("securitySample", null)
                .path("/api")
                .maxAge(0) // ⬅️ הוספת Max-Age=0 מבטלת את העוגייה באופן מיידי
                .build();
        return cookie;
